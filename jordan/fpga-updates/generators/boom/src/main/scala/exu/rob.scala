  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  for (w <- 0 until coreWidth) {
    ...
    //TODO: Remove
    val target_count = RegInit(0.U(64.W))
    //
    ...
    when (io.enq_valids(w)) {
      //TODO: Remove
      when(io.enq_uops(w).debug_inst === 49772643.U){
        target_count := target_count + 1.U
      }
      //
    ...
    //assume 64-bits is enough resolution for branch counters
    val spec_cnt_size = 32 //UInt(64.W)

    //Flag to start counting cycles (start of program/setup procedure) 
    val start_cnt = RegInit(false.B) //TODO: Init to false
    val setup_cnt = RegInit(0.U(spec_cnt_size.W))
    when(start_cnt){
      setup_cnt := setup_cnt + 1.U
    }

    val prev_tail_idx = RegNext(rob_tail_idx)
    //TODO: Make this configurable via a programmable register and search across all dispatched entries
    when(prev_tail_idx =/= rob_tail_idx){
      val temp_idx = prev_tail_idx
      WrapInc(temp_idx, numRobRows)
      for(i <- 0 until coreWidth){
        when(rob_uop(temp_idx).debug_inst === 65811.U){
          //printf("NOP Seen\n")
          start_cnt := true.B
        }
        WrapInc(temp_idx, numRobRows)
    ...
    //Structure that contains information about the speculative branches
    val spec_br_inst        = RegInit(VecInit(Seq.fill(maxBrCount)(0.U(32.W)))) //Don't really want to use this due to it being updated, unless changed to wire
    val spec_br_rob_idx     = RegInit(VecInit(Seq.fill(maxBrCount)(0.U(robAddrSz.W))))
    val spec_br_setup_time  = RegInit(VecInit(Seq.fill(maxBrCount)(0.U(spec_cnt_size.W))))
    val spec_br_cnt         = RegInit(VecInit(Seq.fill(maxBrCount)(0.U(spec_cnt_size.W))))
    val spec_br_val         = RegInit(VecInit(Seq.fill(maxBrCount)(false.B)))
    val spec_br_val_ptr     = RegInit(0.U(log2Ceil(maxBrCount).W))

    dontTouch(start_cnt)
    dontTouch(setup_cnt)
    dontTouch(prev_tail_idx)

    dontTouch(spec_br_inst       )
    dontTouch(spec_br_rob_idx   )
    dontTouch(spec_br_setup_time)
    dontTouch(spec_br_cnt       )
    dontTouch(spec_br_val       )
    dontTouch(spec_br_val_ptr   )
    //Only start tracking branches once the start instruction is seen
    when(start_cnt) {
      //printf("Brupdate B2 Mispredict: %d\n", io.brupdate.b2.mispredict)
      when(io.brupdate.b2.mispredict){
        printf("Brupdate Mispredict UOP: %x PC: %x Inst: %x ROB IDX: %x\n", io.brupdate.b2.uop.uopc, io.brupdate.b2.uop.debug_pc, io.brupdate.b2.uop.inst, io.brupdate.b2.uop.rob_idx)
      }
      //Update all existing speculative branches
      printf("----Target Count: %x----\n", target_count)
      for(i <- 0 until maxBrCount){
          printf("Spec Br " + i + " UOP: %x ROB IDX: %x SETUP: %x CNT: %x VAL: %x", spec_br_inst(i), spec_br_rob_idx(i), spec_br_setup_time(i), spec_br_cnt(i), spec_br_val(i))
          when(spec_br_val(i)){
            printf(" Taken: %d Mispredict: %d ROB Match: %d\n", rob_uop(spec_br_rob_idx(i)).taken, RegNext(io.brupdate.b2.mispredict), RegNext(io.brupdate.b2.uop.rob_idx) === spec_br_rob_idx(i))
            //If mispredicted and the bank index and rob index match the current speculative branch's values 
            when(rob_uop(spec_br_rob_idx(i)).taken && RegNext(io.brupdate.b2.mispredict) && RegNext(io.brupdate.b2.uop.rob_idx) === spec_br_rob_idx(i)) { 
                //MatchBank(GetBankIdx(io.brupdate.b2.uop.rob_idx)) &&                 //If bank matches
                //GetRowIdx(io.brupdate.b2.uop.rob_idx) === spec_br_rob_idx(i)){       //If rob idx matches
              printf("PC: %x, %x Inst: %x Mis-predicted taken with speculative shadow of %x cycles\n", rob_uop(spec_br_rob_idx(i)).uopc, rob_uop(spec_br_rob_idx(i)).debug_pc, rob_uop(spec_br_rob_idx(i)).inst, spec_br_cnt(i))
              
              //Once reported/unreported remove
              spec_br_val(i) := false.B
            }
            //Else if branch marked invalid (misspeculated instruction) [or not busy (resolved)] no longer consider it
            .elsewhen(!rob_val(spec_br_rob_idx(i)) || !rob_bsy(spec_br_rob_idx(i))){
              spec_br_val(i) := false.B
            }
            //Else incremement the speculative shadow count
            .otherwise {
              spec_br_cnt(i) := spec_br_cnt(i) + 1.U
            }
          }
          .otherwise {
            printf("\n")
      ...
      printf("\n")
      //Add new branch(es); add it to the structure by updating an invalid entry
      when(prev_tail_idx =/= rob_tail_idx){
        //printf("Prev Tail not equal to current tail\n")
        val temp_idx = prev_tail_idx
        WrapInc(temp_idx, numRobRows)
        var temp_spec_val_mask = (spec_br_val.asUInt)
        //can get w new instructions every issue/dispatch
        when(rob_uop(temp_idx).is_br && rob_val(temp_idx)){
          //printf("Valid rob entry that is a branch\n")
          for (i <- 0 until coreWidth) {
            val spec_idx = PriorityEncoder(~(temp_spec_val_mask))
            when(spec_br_val(spec_idx) === true.B){
              printf("ERROR: Overwriting Valid Entry")
            }
            spec_br_inst(spec_idx)        := rob_uop(temp_idx).debug_inst
            spec_br_rob_idx(spec_idx)     := temp_idx
            spec_br_setup_time(spec_idx)  := setup_cnt
            spec_br_cnt(spec_idx)         := 0.U
            spec_br_val(spec_idx)         := true.B
            WrapInc(temp_idx, numRobRows)
            temp_spec_val_mask = (temp_spec_val_mask | 1.U << spec_idx)
          }
